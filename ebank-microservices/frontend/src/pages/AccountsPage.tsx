...
function AccountModal({ title, initial, onSubmit, onClose, loading, error }: ModalProps) {
  const [form, setForm] = useState<AccountPayload>(initial)

  const set = (field: keyof AccountPayload, value: string | number) =>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input required value={form.accountNumber}
                onChange={(e) => set('accountNumber', e.target.value)}
                className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">Holder Name *</label>
              <input required value={form.accountHolderName}
            ...
            <div>
              ...
              <select required value={form.accountType}
            ...
            <div>
              ...
              <input required type="number" min="0" step="0.01" value={form.balance}
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage all bank accounts</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <p className="text-slate-500 text-sm text-center py-16">No accounts found.</p>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {accounts.map((acc) => (
                  <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-3 font-mono text-slate-800">{acc.accountNumber}</td>
                    <td className="px-6 py-3 text-slate-800">{acc.accountHolderName}</td>
                    <td className="px-6 py-3 text-slate-600">{acc.accountType}</td>
                    <td className="px-6 py-3 font-semibold text-slate-900">
                      ${acc.balance.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </td>
                    <td className="px-6 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor(acc.status)}`}>
                        {acc.status}
                      </span>
                    </td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => { setEditTarget(acc); setModalError(null) }}
      ...
      {editTarget && (
        <AccountModal
          ...
          initial={{
            ...
            accountHolderName: editTarget.accountHolderName,
      ...
      {deleteConfirm !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            <h3 className="text-slate-900 font-semibold text-lg mb-2">Delete Account</h3>
